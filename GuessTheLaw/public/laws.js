...
export async function getCoordinate(locationName) {
    const formattedName = removeNonAlpabetical(locationName).toLowerCase().trim();
    if (window.localStorage.getItem("coord-" + formattedName) !== null) 
        return JSON.parse(window.localStorage.getItem("coord-" + formattedName)); // read local cache

    const ix = formattedLocations.indexOf(formattedName);
    if (ix >= 0) locationName = locations[ix];
    ...
    if (data.geonames.length > 0) {
        ...
        window.localStorage.setItem("coord-" + formattedName, JSON.stringify({lat: latitude, lng: longitude})); // save local cache
...
export function removeNonAlpabetical(str) {
    const re = /[^A-Z^a-z]/;
    let ix = re.exec(str);
    while (ix !== null) {
        str = replaceAt(str, ix.index, "");
        ix = re.exec(str);
    }
    return str;
}
...
const formattedLocations = [...locations].map(e => removeNonAlpabetical(e).toLowerCase().trim());

/// checks if this is a valid location for the stupid laws
export function isLocation(loc) {
    loc = removeNonAlpabetical(loc).toLowerCase().trim()
    return formattedLocations.some(element => {
        return element === loc;
    });
}
